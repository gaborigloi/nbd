...
let serve_internal t (type t) ?(read_only=true) block (b:t) =
  ...
  let rec loop () =
    ...
    | { command_flags = []; ty = Command.Write; from; len; handle } ->
      (* We do not support the only command flags valid for NBD_CMD_WRITE:
       * we do not send NBD_FLAG_SEND_FUA, so the client MUST NOT send NBD_CMD_FLAG_FUA.
    ...
    | { command_flags = [CommandFlag.NoHole]; ty = Command.WriteZeroes; from; len; handle } ->
      (* The only flag we support for NBD_CMD_WRITE_ZEROES is NBD_CMD_FLAG_NO_HOLE *)
    ...
    | { command_flags = []; ty = Command.Read; from; len; handle } ->
      (* We do not support any of the command flags valid for NBD_CMD_READ: we
       * send neither NBD_FLAG_SEND_FUA, nor NBD_CMD_FLAG_DF, so the client
       * MUST NOTsend the corresponding flags. *)
    ...
    | { ty = Command.Disc; _ } ->
      ...
      Lwt.return_unit
    | { ty = Command.(Write | WriteZeroes | Read); _ } ->
