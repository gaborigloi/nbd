...
module Impl = struct
  ...
  let fail_on_error msg p =
    p >>= function
    | Error e -> Lwt.fail_with (msg ^ ": " ^ (Protocol.OptionError.to_string e))
    | Ok x -> Lwt.return x

  let require_structured_reply c =
    Nbd.Client.negotiate_structured_reply c |> fail_on_error "Failed to negotiate structured replies"
    ...
    let t =
      ...
      Lwt.finalize
        (fun () ->
           ...
           Nbd.Client.list_meta_contexts c export queries |> fail_on_error "Failed to list meta contexts" >>= fun l ->
    ...
    let t =
      ...
      Lwt.finalize
        (fun () ->
           Nbd.Client.connect channel >>= fun c ->
           require_structured_reply c >>= fun () ->
           Nbd.Client.set_meta_contexts c export queries |> fail_on_error "Failed to set meta contexts" >>= fun l ->
           Lwt_io.printl "Got meta contexts:" >>= fun () ->
           Lwt_list.iter_s (fun (id, name) -> Lwt_io.printlf "%ld: %s" id name) l >>= fun () ->
           require_export c export >>= fun (c, _, _block_sizes) ->
           (* Here we assume that the client's request respects the max block size *)
